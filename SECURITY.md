...
| 恶意页面开窗/跳转劫持 | 主窗口与 dshWebView 共用外链守卫（仅放行本机 dsh web 端口，其余 openExternal）；settings/plugins/sessions 窗口统一 `applyOwnWindowLinkPolicy`（https → 系统浏览器，导航拦截） |
...
| 官方数据被改写 | 会话中心对 `~/.dsh` 与官方接口只读；插件安装/更新走官方 `dsh plugin` 命令，桌面壳不直接改写官方存储 |
...
- 产物当前**未签名、未公证**（macOS 首启需右键放行）；签名/公证为已排期后续任务（见 README 路线图）；
- Release 附 SHA256 清单供校验；官网安装引导包含校验步骤；
